										...
										if($ssh->execute("cd /home/skripte && sudo -u ".$dbconnect->HtmlEscape($teamspeakrow["sshuser"])." ./daemon tsdnseintrag $nodeimported $teamspeakordner $subdomain $ip $port")){
											if($ssh->execute("cd /home/skripte && sudo -u ".$dbconnect->HtmlEscape($teamspeakrow["sshuser"])." ./daemon tsdnstop $nodeimported $nodepfad")){
												if($ssh->execute("cd /home/skripte && sudo -u ".$dbconnect->HtmlEscape($teamspeakrow["sshuser"])." ./daemon tsdnsrestart $nodeimported $teamspeakordner")){
													...
													echo"<meta http-equiv=\"refresh\" content=\"0; url=admin.php?op=adminteamspeaktsdnsverwaltung&edit=".$dbconnect->HtmlEscape($_GET["edit"])."\">";
												}else{
											...
											$false = "Der TSDNS Eintrag konnte nicht erstellt werden.";
										}
									}else{
