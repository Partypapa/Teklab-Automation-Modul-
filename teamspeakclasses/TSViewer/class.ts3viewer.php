		...
		if(isset($this->_userDatas[$channelId]))
		{
			...
			foreach ($this->_userDatas[$channelId] as $user)
			{
				if($user["client_type"] == 0)
				{
					...
					$client_unique_identifier = $this->toHTML($user["client_unique_identifier"]);
					$clid = $this->toHTML($user["clid"]);
					$cldbid = $this->toHTML($user["client_database_id"]);
					$edit = $this->toHTML($_GET["edit"]);
...
<div class="tsstatusItem">
	<a href="#" data-container="body" class="userviewer" data-toggle="popover" data-content="Name: $name <br /> Client-ID: $clid <br /> Datenbank-ID: $cldbid"  title="$name / $clid"> <img src="$imagePath$icon" /> $name </a>
	<a href="#" onclick="HideModul('ts3viewer', 'banuser',  '$clid', '$name', 'clid')" style="float:right;margin-top:-17px;margin-right:10px;">Ban</a>
	<a href="#" onclick="HideModul('ts3viewer', 'kickuser', '$clid', '$name', 'kickclientid')" style="float:right;margin-top:-17px;margin-right:100px;">Kick</a>
	<a href="#" onclick="HideModul('ts3viewer', 'pokeuser', '$clid', '$name', 'clientid')" style="float:right;margin-top:-17px;margin-right:70px;">Pock</a>
	<a href="#" onclick="HideModul('ts3viewer', 'moveuser', '$clid', '$name', 'moveclid')" style="float:right;margin-top:-17px;margin-right:36px;">Move</a>
</div>

	<div style="float:right;margin-top:-17px;margin-right:122px;">
		$flags
	</div>
HTML;
				}
		...
		foreach ($this->_channelDatas as $channel)
		{
			if($channel["pid"] == $channelId)
			{
				if($channel["show"])
				{
					...
					$content .= <<<HTML
<div class="tsstatusItem">
	<a href="#" data-container="body" class="channelviewer" data-toggle="popover" data-content="Name: $name" title="Channel-ID [$cid]">
